...
TempFiles.prototype.writeKey = function(key, next) {
  fs.writeFile(this.filenames.key, key, { encoding: 'utf-8', mode: 0o0600 }, next);
};

TempFiles.prototype.writeScript = function(script, next) {
  fs.writeFile(this.filenames.script, script, { encoding: 'utf-8', mode: 0o0700 }, next);
};
  ...
  return function() {
    ...
    fs.unlink(self.filenames.key, function(err) {
      ...
      fs.unlink(self.filenames.script, function(err) {
        if (err) { debug('Could not cleanup script file due to error', err); }

        cb.apply(null, Array.prototype.slice.call(cbArgs));
      });
    });
  };
};
